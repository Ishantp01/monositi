    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          contact_info: {
            phone: builder.contact_info?.phone || "",
            email: builder.contact_info?.email || "",
            address: builder.contact_info?.address || "",
          },
          certifications: builder.certifications || [],
        });
        if (builder.logo) {
          setLogoPreview(builder.logo);
        }
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        navigate(`/admin/builders/${id}`);
      } else {
        toast.error(data.message || "Failed to update builder");
      }
    } catch (error) {
        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => navigate(`/admin/builders/${id}`)}
